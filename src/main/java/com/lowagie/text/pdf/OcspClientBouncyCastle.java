...
public class OcspClientBouncyCastle implements OcspClient {
    ...
    private static OCSPReq generateOCSPRequest(X509Certificate issuerCert, BigInteger serialNumber) throws OCSPException, IOException {
        ...
        try {
            id = new CertificateID(new BcDigestCalculatorProvider().get(CertificateID.HASH_SHA1), new X509CertificateHolder(Certificate.getInstance(issuerCert)), serialNumber);
        } catch (OperatorCreationException e) {
            throw new RuntimeException("HASH_SHA1 not found", e);
        }
        ...
        Extension oids = new Extension(OCSPObjectIdentifiers.id_pkix_ocsp_nonce, false, OCSPObjectIdentifiers.id_pkix_ocsp_nonce.getEncoded());
        Extension values = new Extension(id.getHashAlgOID(), false, new DEROctetString((PdfEncryption.createDocumentId())));
        gen.setRequestExtensions(new Extensions(new Extension[] { oids, values }));
