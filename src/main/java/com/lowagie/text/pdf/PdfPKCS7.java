...
public class PdfPKCS7 {
    ...
    private final Collection certs;
...
//                    ts.validate(certStoreX509, provider);
                    ts.validate(new SignerInformationVerifier(new DefaultCMSSignatureAlgorithmNameGenerator(),
                            new DefaultSignatureAlgorithmIdentifierFinder(), new JcaContentVerifierProviderBuilder().build(certStoreX509), new BcDigestCalculatorProvider()));
                    return true;
                }
                catch (Exception ex) {
                }
...
//            DERObject obj = getExtensionValue(certificate, X509Extensions.AuthorityInfoAccess.getId());
            ...
            for (int i = 0; i < AccessDescriptions.size(); i++) {
                ...
                if ( AccessDescription.size() != 2 ) {
                    ...
                    if ((AccessDescription.getObjectAt(0) instanceof ASN1ObjectIdentifier) && ((ASN1ObjectIdentifier)AccessDescription.getObjectAt(0)).getId().equals("1.3.6.1.5.5.7.48.1")) {
                        ASN1Primitive objectAt = AccessDescription.getObjectAt(1).toASN1Primitive();
                        String AccessLocation =  getStringFromGeneralName(objectAt);
    ...
    public boolean isRevocationValid() {
        ...
        try {
            ...
            CertificateID tis = new CertificateID(new BcDigestCalculatorProvider().get(CertificateID.HASH_SHA1), new X509CertificateHolder(org.bouncycastle.asn1.x509.Certificate.getInstance(isscer)), sigcer.getSerialNumber());
